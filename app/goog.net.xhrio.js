...
goog.scope(function() {
  ...
  var HTTP_SCHEME_PATTERN = goog.net.XhrIo.HTTP_SCHEME_PATTERN;
  ...
  goog.net.XhrIo.prototype.send = function(url, opt_method, opt_content, opt_headers) {
    ...
    if (opt_headers) {
      if (Object.getPrototypeOf(opt_headers) === Object.prototype) {
        ...
        for (var $jscomp$iter$5 = $jscomp.makeIterator(opt_headers.keys()), $jscomp$key$key = $jscomp$iter$5.next(); !$jscomp$key$key.done; $jscomp$key$key = $jscomp$iter$5.next()) {
          var key$29 = $jscomp$key$key.value;
          {
            headers.set(key$29, opt_headers.get(key$29));
          }
        }
      } else {
    ...
    for (var $jscomp$iter$6 = $jscomp.makeIterator(headers), $jscomp$key$$jscomp$destructuring$var33 = $jscomp$iter$6.next(); !$jscomp$key$$jscomp$destructuring$var33.done; $jscomp$key$$jscomp$destructuring$var33 = $jscomp$iter$6.next()) {
      var $jscomp$destructuring$var33 = $jscomp$key$$jscomp$destructuring$var33.value;
      {
        var $jscomp$destructuring$var34 = $jscomp.makeIterator($jscomp$destructuring$var33);
        var key$30 = $jscomp$destructuring$var34.next().value;
        var value = $jscomp$destructuring$var34.next().value;
        {
          this.xhr_.setRequestHeader(key$30, value);
        }
      }
    }
    ...
    if ("setTrustToken" in this.xhr_ && this.trustToken_) {
      try {
        ...
        goog.log.fine(this.logger_, this.formatMsg_("Error SetTrustToken: " + err$31.message));
      }
    }
    try {
      ...
      goog.log.fine(this.logger_, this.formatMsg_("Send error: " + err$32.message));
      this.error_(goog.net.ErrorCode.EXCEPTION, err$32);
    }
  };
  ...
  goog.net.XhrIo.prototype.cleanUpXhr_ = function(opt_fromDispose) {
    if (this.xhr_) {
      this.cleanUpTimeoutTimer_();
      var xhr = this.xhr_;
      var clearedOnReadyStateChange = this.xhrOptions_[goog.net.XmlHttp.OptionType.USE_NULL_FUNCTION] ? function() {
      } : null;
      this.xhr_ = null;
      this.xhrOptions_ = null;
      if (!opt_fromDispose) {
