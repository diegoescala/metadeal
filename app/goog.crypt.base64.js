...
goog.crypt.base64.encodeByteArray = function(input, alphabet) {
  ...
  var output = new Array(Math.floor(input.length / 3));
  var paddingChar = byteToCharMap[64] || "";
  var inputIdx = 0;
  var outputIdx = 0;
  for (; inputIdx < input.length - 2; inputIdx += 3) {
    var byte1 = input[inputIdx];
    var byte2$33 = input[inputIdx + 1];
    var byte3 = input[inputIdx + 2];
    var outChar1 = byteToCharMap[byte1 >> 2];
    var outChar2 = byteToCharMap[(byte1 & 3) << 4 | byte2$33 >> 4];
    var outChar3$34 = byteToCharMap[(byte2$33 & 15) << 2 | byte3 >> 6];
    var outChar4 = byteToCharMap[byte3 & 63];
    output[outputIdx++] = "" + outChar1 + outChar2 + outChar3$34 + outChar4;
  }
  var byte2 = 0;
  var outChar3 = paddingChar;
  switch(input.length - inputIdx) {
    case 2:
      byte2 = input[inputIdx + 1];
      outChar3 = byteToCharMap[(byte2 & 15) << 2] || paddingChar;
    case 1:
      var byte1$35 = input[inputIdx];
      var outChar1$36 = byteToCharMap[byte1$35 >> 2];
      var outChar2$37 = byteToCharMap[(byte1$35 & 3) << 4 | byte2 >> 4];
      output[outputIdx] = "" + outChar1$36 + outChar2$37 + outChar3 + paddingChar;
    default:
  }
  return output.join("");
};
