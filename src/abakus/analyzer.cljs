...
(def params
  {:five-year-price             (fn [p] (* (+ 1.0 (* (:time-horizon-years p) 0.04))
   ...
   :monthly-maint               (fn [p] (/ (:property-value-amt p) 2000.0))
   :num-units                   (fn [p] 1)
   :rent-per-unit               (fn [p] 800)
   :vacancy-percentage          (fn [p] 5.0)
   :property-value-amt          (fn [p] (* 0.5 (+ (:purchase-price p) (:five-year-price p))))
   :cap-rate                    (fn [p] (/ (+ (:annual-profit p) (* 12 (:loan-principal-interest p)))
                                           (:purchase-price p)))
   ...
   :property-tax-and-insurance  (fn [p] (/ (+ (* (:property-value-amt p)
                                                 (* 0.01 (:prop-tax-percent p)))
                                              (:prop-insur p))
                                           12.0))
   :prop-tax-percent            (fn [p] 2.3)
   :prop-insur                  (fn [p] (* (:property-value-amt p) 0.015))
   :monthly-non-mortgage-exp    (fn [p] (- (:monthly-exp p) (:mortgage p)))
...
(defn recompute
  ...
  (->> prop
    ...
    (compute :monthly-exp)
    (compute :monthly-non-mortgage-exp)
